...
//            Arrays.stream(worlds[x].listFiles()).forEach(System.out::println);
            File[] subList=worlds[x].listFiles();
            for(File subFile:subList){
                if(subFile.isDirectory()&&(subFile.getName().startsWith("overworld")))
    ...
    public static ArrayList<String> getUniquePaths(ArrayList<File> files) throws IOException {
        ...
        for(File f:files){
            ...
            path=path.substring(path.indexOf("overworld"));
            int y;
